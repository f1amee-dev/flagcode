...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `flagcode` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@flagcode/shared/git`, `@flagcode/shared/DrainableWorker`) — no barrel index.
