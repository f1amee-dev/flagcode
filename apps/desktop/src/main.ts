...
import type { ContextMenuItem } from "@flagcode/contracts";
import { RotatingFileSink } from "@flagcode/shared/logging";
...
const BASE_DIR = process.env.FLAGCODE_HOME?.trim() || Path.join(OS.homedir(), ".flagcode");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SETTINGS_PATH = Path.join(STATE_DIR, "desktop-settings.json");
const CLIENT_SETTINGS_PATH = Path.join(STATE_DIR, "client-settings.json");
const SAVED_ENVIRONMENT_REGISTRY_PATH = Path.join(STATE_DIR, "saved-environments.json");
const DESKTOP_SCHEME = "flagcode";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "FlagCode (Dev)" : "FlagCode (Alpha)";
const APP_USER_MODEL_ID = isDevelopment ? "com.flagcode.flagcode.dev" : "com.flagcode.flagcode";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "flagcode-dev.desktop" : "flagcode.desktop";
const LINUX_WM_CLASS = isDevelopment ? "flagcode-dev" : "flagcode";
const USER_DATA_DIR_NAME = isDevelopment ? "flagcode-dev" : "flagcode";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "FlagCode (Dev)" : "FlagCode (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.FLAGCODE_PORT;
  delete env.FLAGCODE_MODE;
  delete env.FLAGCODE_NO_BROWSER;
  delete env.FLAGCODE_HOST;
  delete env.FLAGCODE_DESKTOP_WS_URL;
  delete env.FLAGCODE_DESKTOP_LAN_ACCESS;
  delete env.FLAGCODE_DESKTOP_LAN_HOST;
  return env;
}
...
function resolveAdvertisedHostOverride(): string | undefined {
  const override = process.env.FLAGCODE_DESKTOP_LAN_HOST?.trim();
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { flagcodeCommitHash?: unknown };
    return normalizeCommitHash(parsed.flagcodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.FLAGCODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("FlagCode failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const hasUpdateFeedConfig =
    readAppUpdateYml() !== null || Boolean(process.env.FLAGCODE_DESKTOP_MOCK_UPDATES);
  const disabledReason = getAutoUpdateDisabledReason({
    isDevelopment,
    isPackaged: app.isPackaged,
    platform: process.platform,
    appImage: process.env.APPIMAGE,
    disabledByEnv: process.env.FLAGCODE_DISABLE_AUTO_UPDATE === "1",
    hasUpdateFeedConfig,
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `FlagCode ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/FlagCode (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`flagcode`). If the legacy
...
function configureAutoUpdater(): void {
  const githubToken =
    process.env.FLAGCODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
  ...
  if (process.env.FLAGCODE_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.FLAGCODE_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
...
async function bootstrap(): Promise<void> {
  ...
  const configuredBackendPort = resolveConfiguredDesktopBackendPort(process.env.FLAGCODE_PORT);
  if (isDevelopment && configuredBackendPort === undefined) {
    throw new Error("FLAGCODE_PORT is required in desktop development.");
  }
