...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    availableEditors: [],
    observability: {
      logsDirectoryPath: "/repo/project/.flagcode/logs",
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              FLAGCODE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.FLAGCODE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              FLAGCODE_PROJECT_ROOT: "/repo/project",
              FLAGCODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: FlagCode Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
