  ...
  Effect.gen(function* () {
    ...
    const runtime = ManagedRuntime.make(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "flagcode-web",
          attributes: {
            "service.runtime": "flagcode-web",
    ...
    const servedRoutesLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        Layer.mock(TextGeneration)({
          ...
          generatePrContent: () => Effect.succeed({ title: "", body: "" }),
          generateBranchName: () => Effect.succeed({ branch: "" }),
          generateThreadTitle: () => Effect.succeed({ title: "" }),
          generateWriteup: () => Effect.succeed({ writeup: "" }),
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves project favicon requests before the dev URL redirect", () =>
    Effect.gen(function* () {
      ...
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "flagcode-router-project-favicon-",
      });
  ...
  it.effect("serves the fallback project favicon when no icon exists", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "flagcode-router-project-favicon-fallback-",
      });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/flagcode/environment");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const projectDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "flagcode-router-project-favicon-query-token-",
        });
        ...
        resourceSpans: [
          {
            resource: {
              attributes: [
                {
                  key: "service.name",
                  value: { stringValue: "flagcode-web" },
                },
              ],
            },
            scopeSpans: [
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "flagcode-web");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "flagcode-ws-auth-required-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "flagcode-ws-project-search-",
      });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "flagcode-ws-project-write-",
      });
