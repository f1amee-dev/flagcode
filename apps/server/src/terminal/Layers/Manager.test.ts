  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(spawnInput.env.FLAGCODE_PORT).toBeUndefined();
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            FLAGCODE_PROJECT_ROOT: "/repo",
            FLAGCODE_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.FLAGCODE_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.FLAGCODE_WORKTREE_PATH, "/repo/worktree-a");
