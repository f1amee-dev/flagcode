...
import { deepMerge } from "@flagcode/shared/Struct";
...
function withTempCodexHome(configContent?: string) {
  return Effect.gen(function* () {
    ...
    const tmpDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "flagcode-test-codex-" });
    ...
    describe("checkCodexProviderStatus", () => {
        ...
        () =>
          Effect.gen(function* () {
            ...
            const binDir = yield* fileSystem.makeTempDirectoryScoped({
              prefix: "flagcode-test-codex-bin-",
            });
            ...
            const customCodexHome = yield* fileSystem.makeTempDirectoryScoped({
              prefix: "flagcode-test-codex-home-",
            });
      ...
      it.effect("returns unavailable when codex is below the minimum supported version", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            status.message,
            "Codex CLI v0.36.0 is too old for FlagCode. Upgrade to v0.37.0 or newer and restart FlagCode.",
          );
        }).pipe(
          Effect.provide(
            mockSpawnerLayer((args) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("skips codex probes entirely when the provider is disabled", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(status.installed, false);
          assert.strictEqual(status.message, "Codex is disabled in FlagCode settings.");
        }),
      );
    });
