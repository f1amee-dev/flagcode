...
} from "@flagcode/shared/git";

import { GitManagerError } from "@flagcode/contracts";
import {
  GitManager,
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `flagcode/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(tempDir, `flagcode-pr-body-${process.pid}-${randomUUID()}.md`);
    yield* fileSystem
