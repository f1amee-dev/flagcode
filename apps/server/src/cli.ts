...
const BootstrapEnvelopeSchema = Schema.Struct({
  ...
  flagcodeHome: Schema.optional(Schema.String),
...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to FLAGCODE_HOME)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to FLAGCODE_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("FLAGCODE_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("FLAGCODE_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.boolean("FLAGCODE_TRACE_TIMING_ENABLED").pipe(
    Config.withDefault(true),
  ),
  traceFile: Config.string("FLAGCODE_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.int("FLAGCODE_TRACE_MAX_BYTES").pipe(Config.withDefault(10 * 1024 * 1024)),
  traceMaxFiles: Config.int("FLAGCODE_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("FLAGCODE_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(200)),
  otlpTracesUrl: Config.string("FLAGCODE_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("FLAGCODE_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpExportIntervalMs: Config.int("FLAGCODE_OTLP_EXPORT_INTERVAL_MS").pipe(
    Config.withDefault(10_000),
  ),
  otlpServiceName: Config.string("FLAGCODE_OTLP_SERVICE_NAME").pipe(
    Config.withDefault("flagcode-server"),
  ),
  mode: Config.schema(RuntimeMode, "FLAGCODE_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("FLAGCODE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("FLAGCODE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  flagcodeHome: Config.string("FLAGCODE_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("FLAGCODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("FLAGCODE_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("FLAGCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("FLAGCODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
    ...
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(
          ...
          Option.fromUndefinedOr(env.flagcodeHome),
          Option.fromUndefinedOr(bootstrap?.flagcodeHome),
        ),
      ),
    );
...
const serveCommand = Command.make("serve", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription(
    "Run the FlagCode server without opening a browser and print headless pairing details.",
  ),
  Command.withHandler((flags) =>
...
export const cli = Command.make("flagcode", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the FlagCode server."),
  Command.withHandler((flags) => runServerCommand(flags)),
