...
describe("extractPathFromShellOutput", () => {
  it("extracts the path between capture markers", () => {
    expect(
      extractPathFromShellOutput(
        "__FLAGCODE_PATH_START__\n/opt/homebrew/bin:/usr/bin\n__FLAGCODE_PATH_END__\n",
      ),
...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    >(() => "__FLAGCODE_ENV_PATH_START__\n/a:/b\n__FLAGCODE_ENV_PATH_END__\n");
    ...
    expect(args?.[1]).toContain("__FLAGCODE_ENV_PATH_START__");
    expect(args?.[1]).toContain("__FLAGCODE_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__FLAGCODE_ENV_PATH_END__",
        "__FLAGCODE_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__FLAGCODE_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      [
        "__FLAGCODE_ENV_CUSTOM_VAR_START__",
        "  padded value  ",
        "__FLAGCODE_ENV_CUSTOM_VAR_END__",
      ].join("\n"),
    );
