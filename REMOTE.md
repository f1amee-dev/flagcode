...
Use this when you want to connect to a FlagCode server from another device such as a phone, tablet, or separate desktop app.
...
Run the server with `flagcode serve`.

```bash
npx flagcode serve --host "$(tailscale ip -4)"
```

`flagcode serve` starts the server without opening a browser and prints:
...
Use `flagcode serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

> Note
> The GUIs do not currently support adding projects on remote environments.
> For now, use `flagcode project ...` on the server machine instead.
...
1. `flagcode serve` issues a one-time owner pairing token.
...
Use `flagcode auth` to manage access after the initial pairing flow.
...
Use `flagcode auth --help` and the nested subcommand help pages for the full reference.
...
- Use `flagcode auth` to revoke credentials or sessions you no longer trust.
